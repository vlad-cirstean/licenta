class Communication extends EventTarget {
    ...
    send(data) {
        ...
        if (this.isMaster()) {
            this.incomingConnections.forEach(i => i.send(message));
            this.outgoingConnections.forEach(i => i.send(message));
        } else {
    ...
    _handleMessages(connection, data) {
        if (data === 'pskeditor:candidate') {
            ...
            this.dispatchEvent(new CustomEvent('remoteCommand', {detail: ops}));
        }
